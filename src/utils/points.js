...
// New function to calculate core points excluding units with doNotCountTowardsMinimum flag
export const getCorePointsForMinimum = (list) => {
  let points = 0;

  if (list.core) {
    list.core.forEach((unit) => {
      // Skip units that have the doNotCountTowardsMinimum flag set to true
      if (!unit.doNotCountTowardsMinimum) {
        points += getUnitPoints(
          { ...unit, type: "core" },
          {
            armyComposition: list.armyComposition || list.army,
